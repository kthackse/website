...
def get_next_or_past_event(published=True):
    event = (
        Event.objects.filter(published=published, ends_at__gte=timezone.now(), type=EventType.HACKATHON.value)
        .order_by("starts_at")
        .first()
    )
    if event:
        event.passed = False
        return event
    event = Event.objects.filter(published=published, type=EventType.HACKATHON.value).order_by("ends_at").first()
    if event:
        event.passed = True
        return event
    return None
